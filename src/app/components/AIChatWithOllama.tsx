...
import { Send, Bot, User, MessageSquare, AlertCircle, Wifi, WifiOff } from 'lucide-react';
...
export default function AIChatWithOllama() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: 'Hello! I\'m your AI learning assistant powered by Ollama. I can help you with study recommendations, answer academic questions, explain concepts, and create personalized study plans. How can I assist you today?',
      timestamp: new Date(),
    },
  ]);
    ...
    try {
      ...
      if (ollamaConnected) {
        ...
        try {
          ...
          responseText = await ollamaClient.chat(messageText, conversationHistory);
        } catch (ollamaError) {
          console.error('Ollama error, falling back to pattern matching:', ollamaError);
          responseText = getLocalAIResponse(messageText);
          setOllamaConnected(false);
        }
      } else {
        // Try the local Study AI Agent first, then fallback to pattern matching
        try {
          const agentRes = await studyAgentClient.ask(messageText);
          if (agentRes && agentRes.success && agentRes.response) {
            const responseId = (Date.now() + 1).toString();
            responseText = agentRes.response;

            // Ask for quick confirmation
            try {
              const confirmHelp = window.confirm('Was this answer helpful? Click OK to confirm intent, Cancel to open correction controls.');
              if (confirmHelp) {
                if (agentRes.intent) {
                  await studyAgentClient.sendFeedback(messageText, agentRes.intent);
                }
                const saveExample = window.confirm('Save this Q/A to the dataset as an example?');
                if (saveExample) {
                  setSaveOpenId(responseId);
                }
              } else {
                setCorrectOpenId(responseId);
              }
            } catch (e) {
              console.warn('Confirmation flow error:', e);
            }
      ...
      {!ollamaConnected && ollamaConnected !== null && (
        <div className="bg-orange-50 border-b border-orange-200 px-5 py-3">
          <div className="flex items-start gap-2 text-sm">
            ...
            <div>
              <p className="text-orange-800 font-medium">Ollama not connected</p>
              <p className="text-orange-700 text-xs mt-1">
                Using basic responses. To enable AI: Install Ollama, run <code className="bg-orange-100 px-1 rounded">ollama serve</code>, then click Reconnect.
              </p>
            </div>
          </div>
        </div>
